...
static void send_static_request(uint16_t nodeid, unsigned long i)
{
    char req_uri[32];
    ...
    snprintf(req_uri, 32, "/temperature/%.*s/%05lu", 5, addrstr, i - 1);
...
static int _send_get(int argc, char **argv)
{
    ...
    while ((i++) < EVENTS * narr_sz) {
        ...
        msg_t m;
        msg_receive(&m);
        send_static_request(narr[m.content.value], i);
...
//    ccnl_ndntlv_prependSignedContent(pkt->pfx, (unsigned char*) &temp, aeslen, NULL, NULL, hmac_key, (uint8_t *)&keyid, sizeof(keyid), &offs, data_buf, &reslen);
    ccnl_ndntlv_prependSignedContent(pkt->pfx, (unsigned char*) &temp, sizeof(temp), NULL, NULL, hmac_key, (uint8_t *)&keyid, sizeof(keyid), &offs, data_buf, &reslen);
#else
    uint16_t temp = 2124;
